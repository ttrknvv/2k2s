...
// fi - угол в градусах
{
	double fg = fmod(fi, 360.0); // остаток от деления(для получения угла на который поворачиваем)
...
CBlade::CBlade()            
{
	...
	double RoE = 10 * rS;	// радиус окружности в которой движется лопасть
	double d = RoE;		 // расстояние от центра окна до центра окружности
	RS.SetRectD(-d, d, d, -d);					// Область системы в мировых координатах
	RW.SetRect(0, 0, 600, 600);					// Область в окне
	MainPoint.SetRect(-rS, rS, rS, -rS); // область для главной точки
	FirstTop.SetRect(-5, 5, 5, -5); // область для первой верхней точки
	SecondTop.SetRect(-5, 5, 5, -5); // вторая верхняя точка
	FirstBootom.SetRect(-5, 5, 5, -5); //нижняя точка
	SecondBootom.SetRect(-5, 5, 5, -5); // нихняя точка
	EarthOrbit.SetRect(-RoE, RoE, RoE, -RoE); // область, в которой движется лопасть
	fiSB = 215; // 215
	fiFB = 205;			// 205
	fiST = 25;			// 25
	fiFT = 35;//угл положение для FT 35
	wPoint = -8;
	dt = 0.1;
...
//Вычисляет новые координаты лопости в СК через интервал времени dt
{


	double RoV = (EarthOrbit.right - EarthOrbit.left) / 2; // радиус вектор
	double ff = (fiFT / 90.0) * pi; // угод в радианах
	double x = RoV * cos(ff); // координаты новой лопасти
	double y = RoV * sin(ff); // координаты новой лопасти
	FTCoords(0) = x;
	FTCoords(1) = y;
	FTCoords(2) = 1; // третья координата 1

	fiFT += wPoint * dt; // увеличение угла fi
	CMatrix P = CreateRotate2D(fiFT); // матрица поворота
	FTCoords = P * FTCoords; // произведение матрицы поворота на матрицу старых координат
