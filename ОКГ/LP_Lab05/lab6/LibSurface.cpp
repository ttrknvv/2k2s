...
{
	pFunc = pF; // сохраняем функцию для дальнейшего использования
	SpaceRect.SetRectD(RS.left, RS.top, RS.right, RS.bottom); // устанавливаем прямоугольную область МСК
...
// dy - шаг для расчета значений функции f(x,y) оси y
{
	...
	double yH = SpaceRect.top; // заполнение крайних точек 
	...
	for (double x = xL; x <= xH; x += dx)
	{
		...
		MatrF.push_back(VecMatrix); // заполнение всех значений функции поверхности с заданным шагом
	}
}
...
{
	CMatrix MV = CreateViewCoord(ViewPoint(0), ViewPoint(1), ViewPoint(2));	//Матрица(4x4) пересчета   в видовую систему координат 
	CMatrix RC = GetViewPoint();	//проекция камеры на XY	в видовой системе													
	CVecMatrix VecMatrix; // хранение векторов точек проекции
	CMatrix VX(4), V(3); // хранение координат точек после преобразования, хранение координат точки проекции на плоскость
	V(2) = 1;
	...
	for (int i = 0; i < MatrF.size(); i++)
	{
		...
		{
			VX = MatrF[i][j]; // получаем вектор текущих точек
			VX = MV * VX; // преобразуем его в видовые
	...
	ViewRect.SetRectD(xmin, ymax, xmax, ymin);// Определяем область с полученными максимальными и минимальными значениями
	...
	/*В зависимости от положения камеры (RC) относительно граней AD, AC, CB и BD производится сортировка точек в матрице MatrView. 
	Функции сравнения, такие как CompareVecMatrix и вычисление значений Y1, Y2, используются для определения порядка сортировки.
	Это позволяет установить правильное отображение объектов в трехмерном пространстве на плоскости XY видовой системы координат.*/
}
